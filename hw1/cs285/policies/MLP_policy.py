    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        if self.discrete:
            ...
            return distributions.Normal(
                self.mean_net(observation), torch.exp(self.logstd)[None],
                )
        #-------------------------------------------------------
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(
        ...
        obs_tens = torch.tensor(
            observations,
            device=ptu.device,
            dtype=torch.float
        )
        act_tens = torch.tensor(
            actions,
            device=ptu.device,
            dtype=torch.int if self.discrete else torch.float
        )

        # optimize over negative log likelihood
        action_distribution = self(obs_tens)
        loss = -action_distribution.log_prob(act_tens).mean()
        loss.backward()
        self.optimizer.step()
        #-------------------------------------------------------------
        return {
